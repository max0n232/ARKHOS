...
Запрещено:
- Создавать Code Snippets / mu-plugins как workaround для отсутствующего доступа
- Ставить костыли вместо запроса credentials/permissions
- Перебирать инструменты вслепую — сначала проверь что доступно (MEMORY.md, MCP abilities), потом действуй
