...
function readStampApproval(sessionId) {
  if (!sessionId) return false;
  const tokenPath = path.join(HOME, ".claude", "hooks", `.deploy-approval-${sessionId}.json`);
  try {
    const tok = JSON.parse(fs.readFileSync(tokenPath, "utf8"));
    if (tok.sessionId !== sessionId) return false;
    if (!tok.ts || (Date.now() - tok.ts) > STAMP_FRESH_MS) return false;
    return true;
  } catch { return false; }
}
