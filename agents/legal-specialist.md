...
- Stage 03 verify § через MCP (квалификация — не по памяти)
...
### `verification`
- For each citation в draft → MCP verify (`mcp__estonian-law__*`)
- **Если MCP недоступен** (tool error / server down — endpoint может быть мёртв): fallback на WebFetch к `riigiteataja.ee/akt/{id}` (первоисточник). НЕ зацикливаться на ретраях мёртвого MCP, НЕ пропускать verification молча — переключиться на WebFetch и пометить в логе `verified_via: webfetch`.
- Если оба недоступны → halt + пометить citation `UNVERIFIED`, эскалировать (НЕ цитировать по памяти как verified).
- Write `verification_log.md` table (с колонкой источника verification)
- Halt on FAILED / SUPERSEDED / TEXT_MISMATCH (это verdict-значения ответа, не ошибка соединения)
...
- НЕ цитировать § без verification (галлюцинация = критичный риск). Verification = estonian-law MCP, А ЕСЛИ MCP недоступен (tool error / server down) → WebFetch к `riigiteataja.ee/akt/{id}` как fallback. Запрещено цитировать ТОЛЬКО по памяти без любого из двух источников.
...
1. `mcp__estonian-law__*` — primary live Riigi Teataja (⚠️ endpoint может быть мёртв — если tool недоступен, источник #6 WebFetch к riigiteataja.ee становится primary)
